...
window.buildRelicStruct = async function buildRelicStruct(){
  ...
  // Explicit metadata verified against EquipParamAntique.csv; see data/relic-verified-meta.source.md.
  const verifiedRes = await fetch('./data/relic-verified-meta.json', {cache:'no-store'});
  if(!verifiedRes.ok) throw new Error(`relic-verified-meta.json 読込失敗 (${verifiedRes.status})`);
  Object.assign(r, await verifiedRes.json());
  return {r};
};