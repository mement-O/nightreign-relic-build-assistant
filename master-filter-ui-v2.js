...
(async()=>{
  ...
  let ignoreObserver=null;
  let simApplying=false;
  let ignoreApplying=false;
  let simQueued=false;
  let ignoreQueued=false;
  ...
  function effectDisplay(effect){
    return effect.displayNameJa||effect.displayName||effect.name||String(effect.masterKey||'').replace(/^name:/,'');
  }
  function directEntryForLabel(label){
    const n=normalizeLabel(label);
    for(let i=0;i<MAP_ITEMS.length;i++){
      if(n===normalizeLabel(MAP_ITEMS[i]))return {major:'全般',group:'マップ環境',category:'map_environment',sortOrder:120000+i*100};
    }
        ...
        if(res.ok){
          ...
          for(const effect of effects||[]){
            ...
            if(effect.gameMajor&&effect.gameGroup){
              ...
              d=master.classify?.(effect)||directEntryForLabel(effectDisplay(effect));
            }
            if(!d)continue;
  ...
  function observeSim(){
    ...
    if(root&&simObserver)simObserver.observe(root,{childList:true,subtree:true});
  }
  function observeIgnore(){
    const list=$('#ignoreEffectList');
    if(list&&ignoreObserver)ignoreObserver.observe(list,{childList:true,subtree:true});
  }
  function queueSimApply(){
    if(simQueued)return;
    simQueued=true;
  ...
  function queueIgnoreApply(delay=0){
    if(ignoreQueued&&delay===0)return;
    if(delay){setTimeout(()=>queueIgnoreApply(0),delay);return;}
    ignoreQueued=true;
    requestAnimationFrame(()=>{
      ignoreQueued=false;
      applyIgnoreList();
  ...
  function applySimulatorHierarchy(){
    ...
    try{
      ...
      for(const section of categories){
        ...
        const sorted=[...nodes].sort((a,b)=>key(entryForLabel(labelOfSim(a)))-key(entryForLabel(labelOfSim(b))));
        for(let i=0;i<sorted.length;i++){
          if(grid.children[i]!==sorted[i])grid.appendChild(sorted[i]);
        }
        const mapped=sorted.map(node=>({node,e:entryForLabel(labelOfSim(node))})).filter(x=>x.e);
        let last=null;
      ...
      const sortedCats=[...categories].sort((a,b)=>key(entryForLabel(labelOfSim($('.sim-effect',a))))-key(entryForLabel(labelOfSim($('.sim-effect',b)))));
      for(const s of sortedCats)root.appendChild(s);
      let lastMajor=null;
      for(const section of sortedCats){
        const first=$('.sim-effect',section);
        const e=first&&entryForLabel(labelOfSim(first));
        if(e&&e.major!==lastMajor){
          const h=document.createElement('div');
  ...
  function originalCategoryButton(category){
    try{return $(`#ignoreCategoryTabs [data-ignore-category="${CSS.escape(category)}"]`);}catch{return null;}
  }
  function setActiveMasterButton(btn){
    $$('.master-filter-btn').forEach(b=>b.classList.toggle('active',b===btn));
  }
  ...
  function createIgnoreHierarchy(){
    ...
    nav.id='masterIgnoreHierarchy';
    nav.className='master-ignore-hierarchy';

    const all=document.createElement('button');
    all.type='button';
    all.className='category-btn master-filter-btn active';
    all.textContent='すべて';
    all.addEventListener('click',()=>{
      ignoreSelection={mode:'all',major:null,group:null,category:'all'};
      setActiveMasterButton(all);
    ...
    for(const major of master.hierarchy){
      ...
      block.className='master-filter-major';
      const title=document.createElement('div');
      title.className='master-filter-major-title';
      title.textContent=major.label;
      block.appendChild(title);
      const row=document.createElement('div');
      row.className='master-filter-group-row';
      for(const group of major.groups){
        const btn=document.createElement('button');
        btn.type='button';
        btn.className='category-btn master-filter-btn';
  ...
  function applyIgnoreList(){
    const list=$('#ignoreEffectList');
    if(!list||ignoreApplying)return;
    const options=$$('.effect-option',list);
    if(!options.length)return;
    ignoreApplying=true;
    ignoreObserver?.disconnect();
    try{
      const items=options.map((node,index)=>({node,index,label:labelOfIgnore(node),e:entryForLabel(labelOfIgnore(node))}));
      const sorted=[...items].sort((a,b)=>key(a.e)-key(b.e)||a.index-b.index);
      for(const {node,label,e} of sorted){
        let show=true;
        if(ignoreSelection.mode==='group'){
          show=e?(e.major===ignoreSelection.major&&e.group===ignoreSelection.group):fallbackBelongs(label);
        }
        node.style.display=show?'':'none';
      }
      // Reorder only if the current option order is actually different.
      const current=$$('.effect-option',list);
      const desired=sorted.map(x=>x.node);
      const orderDiff=current.length!==desired.length||current.some((n,i)=>n!==desired[i]);
      if(orderDiff){
        for(const node of desired)list.appendChild(node);
      }
      for(const child of [...list.children]){
        if(!child.classList.contains('effect-option')&&!child.classList.contains('master-ignore-empty')){
          child.style.display=ignoreSelection.mode==='all'?'':'none';
        }
      }
      let empty=$('.master-ignore-empty',list);
      const visible=sorted.some(x=>x.node.style.display!=='none');
      if(!visible){
        if(!empty){
          empty=document.createElement('div');
          empty.className='master-ignore-empty muted';
          empty.textContent='この分類に表示できる効果はありません。';
  ...
  function bind(){
    ...
    simObserver=new MutationObserver(()=>{
      if(!simApplying)queueSimApply();
    });
    ignoreObserver=new MutationObserver(()=>{
      if(!ignoreApplying)queueIgnoreApply();
    });
    observeSim();
    observeIgnore();
...
})().catch(err=>console.error('master-filter-ui-v2',err));
